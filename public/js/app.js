...
window.switchTab = function switchTab(tab) {
    ...
    if (tab === 'dashboard') {
        ...
        logsTab.classList.add('border-b-2', 'border-primary', 'text-gray-900');
        logsTab.classList.remove('text-gray-500');
...
function renderWatchdog() {
    ...
    let data = [...lastWatchdogData];
    if (watchdogSortKey) {
        data.sort((a, b) => {
            const av = a[watchdogSortKey] ?? -1;
            const bv = b[watchdogSortKey] ?? -1;
            return watchdogSortAsc ? av - bv : bv - av;
        });
    }
    const avgSuccess = data.reduce((a, b) => a + (b.successRate || 0), 0) / data.length;
    data.forEach(sub => {
    ...
    document.querySelectorAll('.watchdog-sort').forEach(th => {
        ...
        if (indicator) indicator.textContent = key === watchdogSortKey ? (watchdogSortAsc ? ' ↑' : ' ↓') : ' ↕';
    });
}
...
async function loadBlacklistSet() {
    try {
        ...
        blacklistEmailSet = new Set(snap.docs.map(d => d.id));
    } catch (e) { /* silent */ }
}
...
function renderContacts() {
    ...
    const filtered = allContactsCache.filter(({ contact }) => {
        ...
        const isBlocked = blacklistEmailSet.has(normalizeEmail(contact.email));
        const matchFilter =
            filter === 'all' ||
            (filter === 'waiting'     && !contact.sent && !contact.handoff && !isBlocked) ||
            (filter === 'sent'        && contact.sent && !contact.handoff) ||
            (filter === 'handoff'     && contact.handoff) ||
            (filter === 'blacklisted' && isBlocked);
    ...
    filtered.forEach(({ id, contact }) => {
        ...
        const isBlacklisted = blacklistEmailSet.has(normalizeEmail(contact.email));
        let statusBadge;
        if (isBlacklisted) {
            statusBadge = '<span class="px-2 py-1 bg-red-100 text-red-700 text-xs rounded-full font-medium">🚫 Blacklist</span>';
        } else if (contact.handoff) {
...
const EVENT_CONFIG = {
    ...
    delivered:      { label: '📬 Doručené',   cls: 'bg-blue-100 text-blue-700' },
    error:          { label: '❌ Chyba',      cls: 'bg-red-100 text-red-700' },
    bounce:         { label: '↩️ Bounce',     cls: 'bg-orange-100 text-orange-700' },
    spam_complaint: { label: '🚨 Spam',       cls: 'bg-purple-100 text-purple-700' },
    blocked:        { label: '🚫 Blokované',  cls: 'bg-gray-100 text-gray-600' },
    handoff:        { label: '✋ Handoff',    cls: 'bg-indigo-100 text-indigo-700' },
};

async function loadLogs() {
    const tbody = document.getElementById('logsTable');
    if (!tbody) return;
    tbody.innerHTML = `<tr><td colspan="6" class="px-6 py-8 text-center text-gray-400">
        <div class="loader mx-auto"></div><p class="mt-2">Načítavam...</p></td></tr>`;
    try {
...
function renderLogs() {
    ...
    // Sync active filter button via data-logfilter attribute
    document.querySelectorAll('.log-filter-btn').forEach(btn => {
        const isActive = btn.dataset.logfilter === logsFilterState;
...
document.querySelectorAll('.watchdog-sort').forEach(th => {
    th.addEventListener('click', () => {
        const key = th.dataset.sort;
        if (watchdogSortKey === key) {
            watchdogSortAsc = !watchdogSortAsc;
        } else {
            watchdogSortKey = key;
            watchdogSortAsc = false;
        }
        renderWatchdog();
    });
});
